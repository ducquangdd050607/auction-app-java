...
public class AuctionServiceTest {
  ...
  static class FakeAuctionDao implements AuctionDao {
    ...
    @Override
    public List<Auction> findByStatus(
        com.auctionapp.auctionappjava.common.enums.AuctionStatus status) {
  ...
  static class FakeBidDao implements BidDao {
    ...
    public List<BidRankingResponse> findRankingByAuctionId(UUID auctionId) {
      ...
      bids.sort(
          Comparator.comparing(BidTransaction::getAmount, Comparator.reverseOrder())
    ...
    public boolean hasSavedBidForAuction(UUID auctionId, BigDecimal amount) {
      return store.values().stream()
          .anyMatch(b -> auctionId.equals(b.getAuctionId()) && amount.equals(b.getAmount()));
    }
    ...
    public long countBidsByBidderId(UUID bidderId) {
      ...
      return findByBidderId(bidderId).stream().map(BidTransaction::getAuctionId).distinct().count();
    }
    ...
    public List<Long> countBidsInWindowTime(
      ...
      long previousWindow =
          findByAuctionId(auctionId).stream()
              .filter(bid -> bid.getCreatedAt() != null)
              .filter(
                  bid ->
                      !bid.getCreatedAt().isBefore(pastTime)
                          && bid.getCreatedAt().isBefore(fromTime))
              .count();
      long currentWindow =
          findByAuctionId(auctionId).stream()
              .filter(bid -> bid.getCreatedAt() != null)
              .filter(
                  bid ->
                      !bid.getCreatedAt().isBefore(fromTime) && bid.getCreatedAt().isBefore(toTime))
              .count();
  ...
  public void testPlaceBid_tooLow() {
    ...
    PlaceBidRequest req =
        new PlaceBidRequest(auctionId, bidderId, new BigDecimal("105")); // less than 110
