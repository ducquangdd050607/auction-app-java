...
public class AuctionServiceAutoBidTest {
    ...
    public void testConfigureAutoBid_successful() {
        ...
        ConfigureAutoBidRequest request = new ConfigureAutoBidRequest(
                auctionId,
                bidderId,
                new BigDecimal("300"),
                new BigDecimal("20"),
        ...
        AutoBidConfig saved = fakeAutoBidDao.findByAuctionIdAndBidderId(auctionId, bidderId).orElseThrow();
    ...
    public void testPlaceBid_triggersAutoBid() {
        ...
        assertTrue(fakeBidDao.hasSavedAutoBidForAuction(auctionId, autoBidderId, new BigDecimal("130")));
        assertEquals(new BigDecimal("1000"), fakeUserDao.findWalletByUserId(manualBidderId).orElseThrow().getBalance());
        assertEquals(new BigDecimal("870"), fakeUserDao.findWalletByUserId(autoBidderId).orElseThrow().getBalance());
    }
    ...
    private AutoBidConfig autoBidConfig(UUID auctionId, UUID bidderId, String maxBid, String increment) {
