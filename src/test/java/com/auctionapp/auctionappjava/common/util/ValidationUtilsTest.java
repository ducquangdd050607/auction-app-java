...
class ValidationUtilsTest {
    ...
    void requireText_withNull_shouldThrowValidationException() {
        ValidationException ex = assertThrows(ValidationException.class,
                () -> ValidationUtils.requireText(null, "Tên đăng nhập"));
        assertTrue(ex.getMessage().contains("Tên đăng nhập"),
                "Thông báo lỗi phải đề cập tên field");
    }
    ...
    void requireText_withEmpty_shouldThrowValidationException() {
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requireText("", "Email"));
    }
    ...
    void requireText_withBlankOnly_shouldThrowValidationException() {
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requireText("   ", "Mô tả"));
    }
    ...
    void requireEmail_withoutAtSign_shouldThrow() {
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requireEmail("not-an-email"));
    }
    ...
    void requireEmail_withoutDomain_shouldThrow() {
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requireEmail("user@"));
    }
    ...
    void requireEmail_withNull_shouldThrow() {
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requireEmail(null));
    }
    ...
    void requirePositive_withPositiveValue_shouldNotThrow() {
        assertDoesNotThrow(() -> ValidationUtils.requirePositive(new BigDecimal("100000"), "Giá khởi điểm"));
    }
    ...
    void requirePositive_withNull_shouldThrow() {
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requirePositive(null, "Giá trị"));
    }
    ...
    void requireTimeRange_withEndBeforeStart_shouldThrow() {
        ...
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requireTimeRange(start, end));
    }

    @Test
    @DisplayName("requireTimeRange — end bằng start phải ném ValidationException (không thể đấu giá 0 giây)")
    void requireTimeRange_withEqualTimes_shouldThrow() {
        LocalDateTime time = LocalDateTime.now();
        assertThrows(ValidationException.class,
                () -> ValidationUtils.requireTimeRange(time, time));
    }
