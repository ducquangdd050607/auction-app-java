...
public class TimePressureAnalyzer implements AuctionActivityAnalyzer {
  ...
  public TrendSignal analyze(AuctionTrendContext context) {
    long totalSeconds =
        Duration.between(context.auction().getStartTime(), context.auction().getEndTime())
            .toSeconds();

    long secondsLeft = Duration.between(context.now(), context.auction().getEndTime()).toSeconds();
