...
public class AutoBidEngine {
  ...
  public Optional<AutoBidResult> calculateNextBid(
    ...
    if (currentPrice != null && currentLeaderId != null) {
      bidList.add(
          new BidCandidate(
              null, currentLeaderId, currentPrice, defaultIncrement, LocalDateTime.now(), false));
    }
    ...
    bidList.sort(
        Comparator.comparing(BidCandidate::getMaxBid, Comparator.reverseOrder())
  ...
  private List<AutoBidConfig> getValidAutoBidConfigs(List<AutoBidConfig> configs) {
    ...
    for (AutoBidConfig config : configs) {
      ...
      if (config.getAuctionId() == null
          || config.getBidderId() == null
          || config.getMaxBid() == null) {
        continue;
      }
  ...
  private Optional<BidCandidate> findTiedAutoBidCompetitor(
      List<BidCandidate> bidList, BidCandidate currentLeader, UUID currentLeaderId) {
