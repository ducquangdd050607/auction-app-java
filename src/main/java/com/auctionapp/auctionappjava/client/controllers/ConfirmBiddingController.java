...
public class ConfirmBiddingController {
    ...
    void handleTrueConfirm(ActionEvent event) {
        if (txtSetPrice.getText().isEmpty()) {
            ...
            ConfigureAutoBidRequest autoBidPayload = null;
            if (isAutoBidding) {
                autoBidPayload = new ConfigureAutoBidRequest(
                        UUID.fromString(currentAuctionId),
                        UUID.fromString(userId),
            ...
            CompletableFuture.supplyAsync(() -> {
                try {
                    if (finalAutoBidPayload != null) {
                        Response autoBidResponse = Client.getInstance().sendRequest(new Request("CONFIGURE_AUTO_BID", finalAutoBidPayload));
