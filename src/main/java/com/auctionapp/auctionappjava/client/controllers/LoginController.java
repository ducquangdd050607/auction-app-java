...
public class LoginController implements Initializable {
    ...
    void handleConfirm(ActionEvent event) throws IOException {
        if (txtUsername.getText().isEmpty() || txtPassword.getText().isEmpty()) {
            ...
            try {
                ClientContext.getInstance().getApi().login(txtUsername.getText().trim(), txtPassword.getText());
                SceneSwitcherController.NewSceneController(event, "/com/auctionapp/auctionappjava/views/Route.fxml", "Vai trò");
            } catch (RuntimeException ex) {
                lblError.setText(ex.getMessage() == null ? "Không thể đăng nhập" : ex.getMessage());
                lblError.setVisible(true);
                lblError.setTextFill(Color.web("#FF8A80"));
            }
        }
